...
function calculateRanks(array) {
    ...
    const ranks = array.map((value) => sortedArray.indexOf(value) + 1);
    return ranks;
}
...
function calculateSpearmanCorrelation(xArray, yArray) {
    ...
    // Calculate the ranks of x and y values
    const xRanks = calculateRanks(xArray);
    const yRanks = calculateRanks(yArray);

    // Calculate the differences between ranks
    const rankDifferencesSquared = [];
    for (let i = 0; i < n; i++) {
        const difference = xRanks[i] - yRanks[i];
        rankDifferencesSquared.push(difference * difference);
    }

    // Calculate the sum of squared rank differences
    const sumRankDifferencesSquared = rankDifferencesSquared.reduce((acc, current) => acc + current, 0);

    // Calculate the Spearman correlation coefficient
    const correlation = 1 - (6 * sumRankDifferencesSquared) / (n * (n * n - 1));
...
function pearson() {
    var x = getArray("numbers");
    var y = getArray("frequencies");
    var res = calculatePearsonCorrelation(x, y);

    var result = document.getElementById("data-result-area");
    result.innerHTML = `<h2>${res.toFixed(2)}</h2>`;
    console.log(res);
}
...
function spearman() {
    var x = getArray("numbers");
    var y = getArray("frequencies");
    var res = calculateSpearmanCorrelation(x, y);
